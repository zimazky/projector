		...
		<div
			className={
				style === 'normal'
					? styles.day
					: style === 'uncompleted'
						? styles.between_firstplanned_and_actual
						: styles.before_actual_date
			}
			...
			>
				...
				{weather ? (
					<div
						...
						title={
							'temperature: ' +
							plus(weather.temperatureMax) +
							'/' +
							plus(weather.temperatureMin) +
							'\nclouds: ' +
							weather.clouds.toFixed(0) +
							'\nprecipitation: ' +
							weather.pop.toFixed(2) +
							'\nrain: ' +
							weather.rain.toFixed(2) +
							'\nsnow: ' +
							weather.snow.toFixed(2) +
							'\ncount: ' +
							weather.count +
							(weather.isThunderstorm ? '\nThunderstorm' : '')
						}
					>
						<sup>{'🌡️' + plus(weather.temperatureMax)}</sup>
						<sub>{plus(weather.temperatureMin)}</sub>
						<sup>{weather.emoji}</sup>
					</div>
				) : null}
			</div>
			<div
				...
				title={
					'planned: ' +
					plannedBalance.toFixed(2) +
					plus(plannedBalanceChange, 2) +
					'\nactual: ' +
