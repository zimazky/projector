    ...
    <>
      <header>
        <div className={styles.buttonGroup}>
          ...
          <TextField label="Name" error={!!errors.name} {...register('name', { required: true })} />
        </div>
        <Tabs
          value={tab}
          labels={['Main', isCompleted ? null : isRepeat ? 'Repeating' : 'Repeat', 'Location']}
          onChange={(e, tab) => {
            setTab(tab)
          }}
        ></Tabs>
          ...
          <div className={styles.grid}>
            {isRepeat || (
              <DatePicker
                ...
                {...register('start', {
                  required: true,
                  pattern: /^20\d{2}\.(0[1-9]|1[0-2]).(0[1-9]|[1-2]\d|3[01])$/
                })}
              />
            )}
            <TextField label="Time" error={!!errors.time} {...register('time', { pattern: /^([0-1]?\d|2[0-3]):[0-5]\d$/ })} />
            <TextField
              label="Duration"
              disabled={!!watch().end}
              error={!!errors.duration}
              {...register('duration', { pattern: /^(\d+d ?)?\d*(:\d\d)?$/ })}
            />{' '}
            {/* поправить с учетом ограничения часов при указании дней*/}
            {isRepeat || (
              <DatePicker
                label="End date"
            ...
            <TextField label={'Credit'} error={!!errors.credit} {...register('credit', { validate: Calc.validate })} />
            <TextField label="Debit" error={!!errors.debit} {...register('debit', { validate: Calc.validate })} />
          </div>
        </TabPanel>
        <TabPanel value={tab} index={1}>
          <TextField label="Repeat template" error={!!errors.repeat} {...register('repeat', { validate: ZCron.validate })} />
          <TextField
      ...
      <YesCancelConfirmation open={saveAsSingle} onConfirm={handleSaveAsSingle} onClose={() => setSaveAsSingle(false)}>
