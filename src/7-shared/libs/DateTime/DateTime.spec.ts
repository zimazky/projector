...
function test(
	f: (ts: number, tz: number, sw: number) => void,
	timestamp: number,
	timezone: number = 0,
	startweek: number = 1
) {
...
function test2(
	f: (ts1: number, ts2: number, tz: number, sw: number) => void,
	timestamp1: number,
	timestamp2: number,
	timezone: number = 0,
	startweek: number = 1
...
function testYMD(f: (s: string, tz: number) => void, str: string, timezone: number = 0) {
	it(`test YYYY-MM-DD = ${str}, timezone=${timezone.toFixed(1)}`, callf(f, str, timezone))
}
	...
	for (let i = 0; i < 100; i++) {
		...
		test(
			(t, tz) => {
				...
				const ex =
					Y + (M > 9 ? '-' : '-0') + M + (D > 9 ? '-' : '-0') + D + (h > 9 ? 'T' : 'T0') + h + (m > 9 ? ':' : ':0') + m
				expect(wd).toEqual(ex)
			},
			timestamp,
			timezone
