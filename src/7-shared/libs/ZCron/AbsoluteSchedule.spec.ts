...
describe('AbsoluteScheduleHandler nextAfter', () => {
	...
	describe('day of month only', () => {
		...
		it('should handle day list', () => {
			...
			expect(formatTs(AbsoluteScheduleHandler.nextAfter(schedule, ts('2024.01.01'), ts('2024.01.05'), 1000))).toBe(
				'2024.01.10'
			)
			expect(formatTs(AbsoluteScheduleHandler.nextAfter(schedule, ts('2024.01.01'), ts('2024.01.12'), 1000))).toBe(
				'2024.01.20'
			)
		})
	})
