...
const UserAvatar: React.FC<UserAvatarProps> = ({ isLoggedIn, avatarUrl, userName, onClick, size = 'small' }) => {
	if (!isLoggedIn) {
		return null
	}
	...
	const className = [styles.avatar, sizeClass, isClickable ? styles.avatarClickable : ''].filter(Boolean).join(' ')
	...
	if (avatarUrl) {
		return (
			...
			>
				<img src={avatarUrl} alt={userName || 'Google user'} className={styles.avatarImage} />
			</div>
		)
	}
	...
	return (
		<div
			...
			title={userName || 'Google user'}
		>
			{initials ? initials : <div className={styles.placeholderIcon} />}
		</div>
	)
}
