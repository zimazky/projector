...
## 12. Статус реализации (актуализация)

Ниже зафиксирован фактический статус реализации на текущий момент.

### 12.1 Выполнено

- Введена сущность документа:
  - `src/6-entities/Document/model/types.ts`
  - `src/6-entities/Document/model/DocumentSessionStore.ts`
  - `src/6-entities/Document/model/index.ts`
- Реализован единый document-flow:
  - открытие произвольного файла из Google Drive;
  - сохранение в текущий открытый файл (`Save to Google Drive`);
  - сохранение в новый файл (`Save to Google Drive As...`);
  - обновление контекста открытого документа после `Save As`.
- `StorageService` переведен на роль сериализации/десериализации:
  - `applyContent(content)` с runtime-валидацией структуры;
  - удален legacy-flow через фиксированный `data.json`.
- Полностью удален legacy `RemoteStorage`:
  - файл `src/7-shared/services/remoteStorage.ts` удален;
  - вызовы `saveToGoogleDrive/loadFromGoogleDrive*` из `StorageService` удалены.
- Интеграция в приложение завершена:
  - `documentSessionStore` подключен в `root`, `StoreContext`, `MainStore`.
- Добавлены операции жизненного цикла документа:
  - `Новый документ`;
  - `Закрыть документ`.
- Реализована защита от потери изменений через UI-диалог:
  - варианты: `Сохранить / Не сохранять / Отмена`;
  - применяется перед `Новый`, `Закрыть`, `Открыть`, `Открыть последний`.
- Добавлена UI-индикация:
  - имя активного документа в `Header`;
  - индикатор dirty (`*`) для сохранения в Drive.
- Реализована persistence-сессия документа в localStorage:
  - ключ `lastOpenedDocument`;
  - сохранение snapshot при `open/save/saveAs/new`;
  - очистка при `close`.
- Восстановление после перезапуска:
  - автопопытка восстановления выполняется после готовности GAPI.
- Исправлены критичные технические проблемы:
  - устранена гонка `logIn()` при параллельных запросах;
  - устранен popup-blocking на старте (логин только по пользовательскому действию).

### 12.2 Частично выполнено / уточнено

- Автовосстановление последнего документа при старте:
  - выполняется только если пользователь уже авторизован;
  - если не авторизован, документ восстанавливается по кнопке `Load from Google Drive` (сначала login, затем load).

### 12.3 Осталось (рекомендованные доработки)

- Полировка UX-текстов и терминологии меню (консистентность языка и названий команд).
- Опционально вынести диалог несохраненных изменений из `CalendarIconBar` в отдельный reusable feature.
- Прогнать ручной regression-check по edge-cases:
  - отказ в логине;
  - 403/404 при открытии файла;
  - конфликты при `Save As`;
  - проверка dirty-state в сложных сценариях последовательных действий.

### 12.4 Обновление критериев готовности

Критерии из раздела 9 считаются достигнутыми.
Дополнительно учесть текущее поведение старта:
- если пользователь не авторизован на момент старта, автоматическая загрузка файла не форсирует login popup;
- восстановление выполняется по явному пользовательскому действию `Load from Google Drive`.
